...
sky { // visual music synth
    ...
    input { // phone and tablet pencil input
        ...
        tilt(0…1~1)
        force(0…0.5 = 0.173918) >> sky.draw.brush.size
    }
    draw { // draw on metal layer
        ...
        brush { // type of brush and range
            size(1…64~10 = 22.913632) // range of radius
...
shader {
    ...
    cell {
        ...
        fade(1.2…3 = 1.239087) >> shader.cell.fade.on(1) {
            on(%2 = 1) >> (shader.cell.fade.on(0),
                           shader.cell.ave.on(0),
                           shader.cell.melt.on(0),
                           shader.cell.tunl.on(0),
                           shader.cell.slide.on(0),
        ...
        ave(0…1 = 0.491803) >> shader.cell.ave.on(1) {
        ...
        melt(0…1 = 0.508197) >> shader.cell.melt.on(1) {
            on(%2 = 0) >> (shader.cell.fade.on(0), shader.cell.ave.on(0), shader.cell.melt.on(0), shader.cell.tunl.on(0), shader.cell.slide.on(0), shader.cell.fred.on(0), shader.cell.zha.on(0))
        }
        tunl(0…5 = 1) >> shader.cell.tunl.on(1) {
            on(%2 = 0) >> (shader.cell.fade.on(0), shader.cell.ave.on(0), shader.cell.melt.on(0), shader.cell.tunl.on(0), shader.cell.slide.on(0), shader.cell.fred.on(0), shader.cell.zha.on(0))
        }
        slide(0…7 = 6.125) >> shader.cell.slide.on(1) {
    ...
    compute {
        ...
        color(y 0…1~0.1 = 0.30122)
    }
    render {
        flatmap {
            frame(x 0, y 0, w 1080, h 1920)
            repeat(x -1…1~0 = 0, y -1…1~0 = 0)
...
model {
    canvas {
        tile {
            ...
            repeat(x -1…1~0 = 0, y -1…1~0 = 0) <> (shader.render.flatmap.repeat, shader.render.cubemap.repeat) >> (midi.cc.skypad.repeatX(val x), midi.cc.skypad.repeatY(val y)) ^ sky.main.anim
        }
        color {
            fade(x 0…1~0.5 = 0.176829, y 0…1~0.1 = 0.30122) >> (shader.compute.color, sky.color.xfade) ^ sky.main.anim
            fill_0(%1 = 0) <> sky.draw.screen.fill(0)
            fill_1(%1 = 0) <> sky.draw.screen.fill(1)
        }
        speed {
    ...
    brush {
        size(0…1~0.5 = 0.347835) <> sky.draw.brush.size
    ...
    cell {
        ...
        ave(0…1~0.5 = 0.491803) <> shader.cell.ave
        melt(0…1~0.5 = 0.508197) <> shader.cell.melt
        tunl(0_5~1) <> shader.cell.tunl
        zha(0_6~2) <> shader.cell.zha
        slide(0_7~3 = 7) <> shader.cell.slide
...
midi { // musical instrument device interface
    ...
    cc {
        skypad {
            ...
            repeatX(cc == 14, val 0_127 = 64) <> model.canvas.tile.repeat(x val)
